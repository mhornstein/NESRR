...
class BERT_Classifier(nn.Module):
    ...
    def __init__(self, input_dim, hidden_layers_config):
        ...
        self.model = create_network(input_dim=input_dim, hidden_layers_config=hidden_layers_config, output_dim=1, include_sigmoid=True)
...
def create_data_loader(X, y, batch_size, shuffle):
    ...
    y_tensor = torch.tensor(y, dtype=torch.float32).unsqueeze(dim=1).to(device)
...
def test_model(model, test_dataloader, df, criterion, output_dir):
    ...
    with torch.no_grad():
        for batch_i, (test_sent_ids, test_embeddings, test_targets) in enumerate(test_dataloader, start=1):
            ...
            predicted_score = test_outputs

            test_predictions = torch.round(test_outputs)
            is_correct = test_targets == test_predictions

            all_test_targets += test_targets.squeeze().int().tolist()
            all_test_predictions += test_predictions.squeeze().int().tolist()
